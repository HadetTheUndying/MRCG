...
integer falloff_damage = 25; //damage value for all hits beyond [falloff_range]

integer LBA_damage = 1; //anti-armor damage dealt to LBA objects

float falloff_range = 70.0; //range in meters at which bullets begin to deal diminished damage. Set to a negative value to invert the falloff, having the [falloff_damage] occur between you and the [falloff_range], rather than between the [falloff_range] and the [max_range]
